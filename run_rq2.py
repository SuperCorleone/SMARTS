...
# Drift scenarios — same intervals as run_rq1.py:DRIFT_SCENARIOS, restricted
# to the three intensities required by RQ2 (transient bursts of 50 samples,
# stream length = 462). 'none' = no drift = TUNE-protocol baseline.
DRIFT_SCENARIOS = {
    'none': [],
    '1x':   [(150, 200)],
...
def apply_drift(y, scenario):
    """Selection A: trigger AND online-update labels both follow the drifted
    stream. Returns a new array with intervals label-flipped."""
    if scenario == 'none' or not DRIFT_SCENARIOS.get(scenario):
        return np.asarray(y).copy()
    y_drifted = np.asarray(y).copy()
    for s, e in DRIFT_SCENARIOS[scenario]:
        y_drifted[s:e] = 1 - y_drifted[s:e]
    return y_drifted


def drift_tag(scenario):
    """Filename fragment for a drift scenario. 'none' -> '' (back-compat)."""
    return '' if scenario == 'none' else f"_drift{scenario}"


def oracle_label_tag(source):
    """Filename fragment for oracle label source."""
    return f"_oraclelabels{source}"


def resolve_oracle_labels(y_valid, drift_scenario, label_source):
    """Return oracle labels according to the selected evaluation protocol."""
    if label_source == 'original' or drift_scenario == 'none':
        return np.asarray(y_valid).copy()
    if label_source == 'match-drift':
        return apply_drift(y_valid, drift_scenario)
    raise ValueError(f"unknown oracle label source: {label_source!r}")


def build_eval_oracle(data, args):
    """Build the evaluation oracle with the requested label-source policy."""
    y_oracle = resolve_oracle_labels(
        data['y_valid'], args.drift_scenario, args.oracle_label_source)
    return build_oracle(data['X_valid'], y_oracle,
...
def phase_setup(args):
    ...
    dtag = drift_tag(args.drift_scenario)
    X_warmup, y_warmup, X_stream, y_stream, X_valid, y_valid = \
        build_data_splits(base_dir)

    # Selection A: trigger AND online_update follow the drifted stream.
    y_stream_drifted = apply_drift(y_stream, args.drift_scenario)
    y_valid_oracle = resolve_oracle_labels(
        y_valid, args.drift_scenario, args.oracle_label_source)

    print(f"=== RQ2 Setup A drift={args.drift_scenario} ===")
    print(f"Warmup: {len(y_warmup)}, Stream: {len(y_stream)}, "
          f"flips: {int((y_stream_drifted != y_stream).sum())}")
    print(f"Oracle labels: {args.oracle_label_source} "
          f"(flips: {int((y_valid_oracle != y_valid).sum())})")
    ...
    # Identify hazard=1 trigger samples on the DRIFTED stream (selection A)
    hazard0_indices = [t for t in range(len(y_stream_drifted))
                       if y_stream_drifted[t] == 1]
    if args.limit is not None:
        hazard0_indices = hazard0_indices[:args.limit]
    print(f"Hazard=1 trigger samples (post-drift): {len(hazard0_indices)}")
    ...
    pkl_path = os.path.join(cd, f'rq2_A{dtag}_setup.pkl')
    with open(pkl_path, 'wb') as f:
        pickle.dump({
            ...
            'y_valid_oracle': y_valid_oracle,
            'X_stream': X_stream,
            'y_stream': y_stream,                # original (kept for reference)
            'y_stream_drifted': y_stream_drifted,  # actual stream used downstream
            'drift_scenario': args.drift_scenario,
            'oracle_label_source': args.oracle_label_source,
...
def phase_online(args):
    ...
    Output is per-(drift, threshold): snapshot/diag filenames are scoped to
    both the drift scenario and the drift-detection threshold.
    """
    base_dir = os.path.dirname(os.path.abspath(__file__))
    cd = cache_dir(base_dir)
    ...
    # Selection A: use the drifted stream for online_update.
    y_stream = data.get('y_stream_drifted', data['y_stream'])
    hazard0_set = set(data['hazard0_indices'])

    print(f"=== RQ2 Online A drift={args.drift_scenario} [{tag}]: "
          f"{len(y_stream)} samples, {len(hazard0_set)} snapshots (no GA) ===")
    t0 = time.time()
    ...
    snap_path = os.path.join(cd, f'rq2_A{dtag}_snapshots_{tag}.pkl')
    with open(snap_path, 'wb') as f:
        pickle.dump(snapshots, f)

    # Save diagnostics
    diag_path = os.path.join(cd, f'rq2_A{dtag}_online_diag_{tag}.json')
...
def phase_online_ga(args):
    ...
    print(f"Building {args.oracle} oracle [{args.oracle_label_source} labels]...")
    oracle = build_eval_oracle(data, args)
    ...
    print(f"=== RQ2 online_ga A drift={args.drift_scenario} [{tag}] "
          f"chunk {args.chunk}/{args.n_chunks}: "
    ...
    oltag = oracle_label_tag(args.oracle_label_source)
    out = os.path.join(cd,
        f'rq2_A{dtag}_online_{tag}_{otag}{oltag}_chunk{args.chunk}.json')
    with open(out, 'w') as f:
        json.dump({'results': results, 'elapsed': round(elapsed, 2),
                   ...
                   'drift_scenario': args.drift_scenario,
                   'oracle': args.oracle,
                   'oracle_label_source': args.oracle_label_source}, f)
    print(f"Saved: {out}")
...
def phase_batch(args, method_key):
    ...
    Per-(drift, oracle): offline Stacking does not perform online updates,
    but the trigger set IS drift-dependent (selection A), so output is keyed
    on drift scenario + oracle.
    """
    base_dir = os.path.dirname(os.path.abspath(__file__))
    cd = cache_dir(base_dir)
    dtag = drift_tag(args.drift_scenario)
    ...
    print(f"=== RQ2 {method_key} A drift={args.drift_scenario} "
          f"chunk {args.chunk}/{args.n_chunks}: "
          f"samples {start}-{end-1} ({len(my_indices)} GA runs, {args.ga_workers} workers) ===")
    t0 = time.time()
    results = {}
    ...
    oltag = oracle_label_tag(args.oracle_label_source)
    out = os.path.join(cd,
        f'rq2_A{dtag}_{method_key}_{otag}{oltag}_chunk{args.chunk}.json')
    with open(out, 'w') as f:
        json.dump({'results': results, 'elapsed': round(elapsed, 2),
                   'chunk': args.chunk, 'n_chunks': args.n_chunks,
                   'drift_scenario': args.drift_scenario,
...
def phase_merge(args):
    """Merge online + bma_online + best_logit chunks for one (drift, threshold, oracle)."""
    base_dir = os.path.dirname(os.path.abspath(__file__))
    cd = cache_dir(base_dir)
    tag = threshold_tag(args.drift_threshold)
    dtag = drift_tag(args.drift_scenario)
    ...
    otag = oracle_tag(args.oracle)
    oltag = oracle_label_tag(oracle_label_source)
    online_results     = _load_chunks(f'rq2_A{dtag}_online_{tag}_{otag}{oltag}_chunk')
    bma_online_results = _load_chunks(f'rq2_A{dtag}_bma_online_{tag}_{otag}{oltag}_chunk')
    best_logit_results = _load_chunks(f'rq2_A{dtag}_best_logit_{otag}{oltag}_chunk')
    ...
    print(f"=== RQ2 Merge A drift={args.drift_scenario} [{tag}]: "
          f"online={len(online_results)}, "
    ...
    output = {
        ...
        'oracle': oracle_kind,
        'oracle_label_source': oracle_label_source,
        'metric_key': err_key,
        'methods': methods, 'diagnostics': online_diag}

    json_path = os.path.join(log_dir, f'rq2_A{dtag}_{tag}_{otag}{oltag}.json')
    with open(json_path, 'w') as f:
        json.dump(output, f, indent=2)
    print(f"Saved: {json_path}")

    tsv_path = os.path.join(log_dir, f'rq2_A{dtag}_{tag}_{otag}{oltag}_summary.tsv')
...
def phase_bma_online(args):
    ...
    Per-drift: BMA uses no drift detection itself, but the online_update label
    stream IS drift-dependent (selection A), so the snapshot file is keyed
    on the drift scenario.
    """
    base_dir = os.path.dirname(os.path.abspath(__file__))
    cd = cache_dir(base_dir)
    dtag = drift_tag(args.drift_scenario)
    ...
    bma_snap_path = os.path.join(
        cd, f'rq2_A{dtag}_bma_snapshots_re{args.retrain_every}.pkl')

    print(f"=== RQ2 BMA-Online A drift={args.drift_scenario} "
...
def phase_bma_online_ga(args):
    ...
    Per-(drift, threshold, oracle): cache + output filenames are scoped to all three.
    """
    base_dir = os.path.dirname(os.path.abspath(__file__))
    cd = cache_dir(base_dir)
    tag = threshold_tag(args.drift_threshold)
    ...
    print(f"=== RQ2 bma_online_ga A drift={args.drift_scenario} [{tag}] "
          f"chunk {args.chunk}/{args.n_chunks}: "
          f"samples {start}-{end - 1} ({len(my_indices)} GA runs, {args.ga_workers} workers) ===")
    t0 = time.time()
    results = {}
    ...
    oltag = oracle_label_tag(args.oracle_label_source)
    out = os.path.join(cd,
        f'rq2_A{dtag}_bma_online_{tag}_{otag}{oltag}_chunk{args.chunk}.json')
    with open(out, 'w') as f:
        json.dump({'results': results, 'elapsed': round(elapsed, 2),
                   'chunk': args.chunk, 'n_chunks': args.n_chunks,
                   'drift_threshold': args.drift_threshold,
...
def phase_best_logit(args):
    ...
    model trained on the warmup set. Per-(drift, oracle): cache + output
    are scoped to drift scenario and oracle (the trigger set is drift-dependent).
    """
    base_dir = os.path.dirname(os.path.abspath(__file__))
    cd = cache_dir(base_dir)
    dtag = drift_tag(args.drift_scenario)
    ...
    print(f"=== RQ2 best_logit A drift={args.drift_scenario} "
          f"chunk {args.chunk}/{args.n_chunks}: "
          f"samples {start}-{end-1} ({len(my_indices)} GA runs, {args.ga_workers} workers) ===")
    t0 = time.time()
    results = {}
    ...
    oltag = oracle_label_tag(args.oracle_label_source)
    out = os.path.join(cd,
        f'rq2_A{dtag}_best_logit_{otag}{oltag}_chunk{args.chunk}.json')
    with open(out, 'w') as f:
        json.dump({'results': results, 'elapsed': round(elapsed, 2),
                   'chunk': args.chunk, 'n_chunks': args.n_chunks,
                   'drift_scenario': args.drift_scenario,
...
def run_single_node(args):
    ...
    print(f"=== RQ2 Setting A drift={args.drift_scenario} [{tag}] "
          f"(single-node, oracle={args.oracle}) ===")
    print(f"Warmup: {len(y_warmup)}, Stream: {len(y_stream)} "
          f"(drift flips: {int((y_stream_drifted != y_stream).sum())}), "
    ...
    print(f"Building {args.oracle} oracle [{args.oracle_label_source} labels]...")
    oracle = build_oracle(X_valid, y_valid_oracle,
                          kind=args.oracle, random_state=args.seed)

    # Trigger on the DRIFTED stream (selection A)
    hazard0_indices = [t for t in range(len(y_stream_drifted))
                       if y_stream_drifted[t] == 1]
    if args.limit:
        hazard0_indices = hazard0_indices[:args.limit]
    ...
    hazard0_rows = {t: add_constant(X_stream.iloc[t:t+1], has_constant='add')
                    for t in hazard0_indices}
    print(f"Hazard=1 triggers (post-drift): {len(hazard0_indices)}")
    ...
    stacking_snaps = {}
    bma_snaps = {}
    for t in range(len(y_stream_drifted)):
        x_raw = X_stream.iloc[t:t+1]
        if t in hazard0_set:
            stacking_snaps[t] = (
        ...
        stacking_online.online_update(x_raw, float(y_stream_drifted[t]))
        bma_online.online_update(x_raw, float(y_stream_drifted[t]))
    ...
    with open(tsv_path, 'w') as f:
        ...
        for m, r in methods.items():
            f.write(f"{m}\t{r['precision']}\t"
                    f"{r[f'mean_{err_key}']}\t{r[f'median_{err_key}']}\t"
                    f"{r['n_samples']}\n")
    print(f"Saved: {json_path}, {tsv_path}")
...
def main():
    ...
    parser.add_argument('--oracle-label-source',
                        choices=['match-drift', 'original'],
                        default='match-drift',
                        help='Oracle training labels for RQ2 evaluation. '
                             'match-drift (default): apply the same drift intervals '
                             'to oracle labels as the evaluated stream. '
                             'original: keep the validation labels unchanged.')
    parser.add_argument('--drift-scenario',
                        choices=list(DRIFT_SCENARIOS.keys()),
                        default='none',
                        help='Drift scenario for the validation stream (selection A: '
                             'trigger and online_update both follow the drifted labels). '
                             'none (default, no drift), 1x, 3x, 6x.')
    # Single-node mode
